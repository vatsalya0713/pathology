    ...
    const fetchPayments = async () => {
      try {
        ...
        const res = await axios.get(
          "http://localhost:8060/api/finance/payments",
        ...
        if (res.data.success) {
          ...
          const total = data.reduce(
            (sum, p) => sum + (Number(p.paidAmt) || 0),
            0,
          );
          const todayStr = new Date().toLocaleDateString();
          const today = data
            .filter((p) => new Date(p.Date).toLocaleDateString() === todayStr)
            .reduce((sum, p) => sum + (Number(p.paidAmt) || 0), 0);
                  ...
                  filteredPayments.map((p, index) => (
                    <tr key={index} className="hover:bg-slate-50 transition">
                      ...
                      <td className="px-4 py-3 font-mono text-sm">
                        <button
                          onClick={() => navigate(`/home/finance/payment/${p.bill_id}`)}
